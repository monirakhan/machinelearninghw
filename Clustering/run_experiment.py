...
if __name__ == '__main__':
    ...
    datasets = []
    dataset1_details = {
            'data': loader.ChessData(verbose=verbose, seed=seed),
            'name': 'chess_dataset',
            'readable_name': 'Chess Dataset',
            'best_nn_params': {'NN__activation': ['tanh'], 'NN__alpha': [1.0],
                               'NN__hidden_layer_sizes': [(36, 36)], 'NN__learning_rate_init': [0.016]}
        }
    dataset2_details = {
            'data': loader.BreastCancerData(verbose=verbose, seed=seed),
            'name': 'breast_cancer_data',
            'readable_name': 'BreastCancerData',
