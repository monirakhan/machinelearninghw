...
def initialize_instances():
    ...
    with open(INPUT_FILE, "r") as abalone:
        ...
        reader = list(abalone.readlines())
        random.shuffle(reader)

        for row in reader:
            data = [float(el.strip()) for el in row.split(",")]
            instance = Instance(data[:-1])
            instance.setLabel(Instance(data[-1]))
...
def split_instances(instances, split=.6):
    training_split = int(split * len(instances))
    training_instances = instances[:training_split]
    testing_instances = instances[training_split:]
...
def train(oa, network, oaName, instances, measure):
    ...
    for iteration in xrange(TRAINING_ITERATIONS):
        ...
        #print "%0.03f" % error
    print ("%0.03f" % (float(correct)/(correct+incorrect)*100.0))
...
def main():
    ...
    for i, name in enumerate(oa_names):
        for split in (data_splits):
            results = ""
            print("Testing accuracies for %s with %f data" % (name, split))
            training_instances, testing_instances = split_instances(instances, split)
            start = time.time()
            correct = 0
            incorrect = 0
