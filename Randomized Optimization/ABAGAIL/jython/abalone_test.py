...
def initialize_instances():
    ...
    with open(INPUT_FILE, "r") as abalone:
        ...
        #     instances.append(instance)
    training_split = int(0.6 * len(instances))
    training_instances = instances[:training_split]
    testing_instances = instances[training_split:]

    return instances, training_instances, testing_instances
...
def main():
    ...
    instances, training_instances, testing_instances = initialize_instances()
    ...
    for i, name in enumerate(oa_names):
        print("Testing accuracies for %s" % name)
        start = time.time()
        correct = 0
        incorrect = 0
